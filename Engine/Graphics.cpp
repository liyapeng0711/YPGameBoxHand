...
void Graphics::DrawRectDimWithPadding(int x, int y, int width, int height, Color c, int padding)
{
	assert(width > 2 * padding);
	assert(height > 2 * padding);
	DrawRectDim(x + padding, y + padding, width - 2 * padding, height - 2 * padding, c);
}
...
void Graphics::DrawIsoRightTriBR(int x, int y, int size, Color c)
{
	...
	for (int row = 0; row < size; row++)
	{
		for (int col = (size-row-1); col < size; col++)
		{
			PutPixel(x0 + col, y0 + row, c);
		}
	}
